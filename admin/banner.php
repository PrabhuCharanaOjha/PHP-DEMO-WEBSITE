...
include('includs/header.php');
  ...
  if (isset($_POST['upload'])) {
    ...
    else 
    {
      ...
      $folder = "image/banner/".$filename; 
            
      $sql = "INSERT INTO banner(heading,description,image) VALUES ('$Heading','$Description','$filename')"; 
      ...
      <div class="row">
        <div class="col-sm-6">
          <div class=" mx-5 my-3 text-center">
            <form action="" class="jumbotron shadow text-dark border mt-3" method="POST" enctype="multipart/form-data">
              <h3 class="text-center mb-3">ADD BANNER</h3>
        ...
        <div class="col-sm-6">
          ...
          if($result->num_rows>0)
          {
              echo '<table class="table table-striped">';
                  ...
                  echo '<tbody>';
                      ...
                      echo '<td>';
                      echo '<img src="image/banner/'.$row['image'].'"width="60" height="60" alt="">';
          ...
          if(isset($_REQUEST['Delete']))
            {
              $hId=$_REQUEST['hId'];
              $sql1="SELECT * FROM banner WHERE id='$hId'";
              $result=$conn->query($sql1);
              if($row1=$result->fetch_assoc())
              {
                unlink("image/banner/".$row1['image']);
                $sql2="DELETE FROM banner WHERE id='$hId'";
                if($conn->query($sql2))
                {
                  echo 'done';
                }
