...
include('includs/header.php');
  ...
  if (isset($_POST['upload'])) {
    if(($_REQUEST['Heading']=="")||($_REQUEST['Description']=="")||($_REQUEST['Date']=="")||($_FILES["uploadfile"]["name"]==""))
    {
    ...
    else 
    {
      ...
      $folder = "image/event/".$filename; 
            
      $sql = "INSERT INTO event(heading,description,date,image) VALUES ('$Heading','$Description','$Date','$filename')"; 
      ...
      <div class="row">
        ...
        <div class="col-sm-6">
          ...
          if($result->num_rows>0)
          {
              echo '<table class="table table-striped">';
                  ...
                  echo '<tbody>';
                      ...
                      echo '<td>';
                      echo '<img src="image/event/'.$row['image'].'"width="60" height="60" alt="">';
          ...
          if(isset($_REQUEST['Delete']))
            {
              $hId=$_REQUEST['hId'];
              $sql1="SELECT * FROM event WHERE id='$hId'";
              $result=$conn->query($sql1);
              if($row1=$result->fetch_assoc())
              {
                unlink("image/event/".$row1['image']);
                $sql2="DELETE FROM event WHERE id='$hId'";
                if($conn->query($sql2))
                {
                  echo 'done';
                }
